...
sudo install -d -m 0755 /etc/myra
sudo cp "$tmp_file" /etc/myra/allowed_domains.txt
sudo chown root:root /etc/myra/allowed_domains.txt
sudo chmod 0444 /etc/myra/allowed_domains.txt
rm -f "$tmp_file"
