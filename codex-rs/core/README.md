...
This crate implements the business logic for Myra. It is designed to be used by the various Myra UIs written in Rust.
...
pointer file), the resolved `gitdir:` target, and `.myra` read-only.
...
`bwrap` is missing, it falls back to the bundled `myra-resources/bwrap`
binary shipped with Myra and Myra surfaces a startup warning through its
normal notification path instead of printing directly from the sandbox helper.
Myra also surfaces a startup warning when bubblewrap cannot create user
...
namespaces, so Myra rejects sandboxed shell commands that would enter the
