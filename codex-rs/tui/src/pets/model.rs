...
//! in-memory `Pet`: built-in catalog ids, custom pet ids under MYRA_HOME,
...
impl Pet {
    ...
    /// has already been materialized into MYRA_HOME; if callers skip the
...
fn load_builtin_pet(pet: catalog::BuiltinPet, codex_home: Option<&Path>) -> Result<Pet> {
    let codex_home = codex_home.context("MYRA_HOME is not available")?;
