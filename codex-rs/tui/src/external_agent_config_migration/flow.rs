...
pub(crate) const EXTERNAL_AGENT_CONFIG_MIGRATION_REMOTE_UNAVAILABLE_MESSAGE: &str =
    "Import from other apps is unavailable in remote sessions. Start Myra locally and run /import.";
pub(crate) const EXTERNAL_AGENT_CONFIG_MIGRATION_DAEMON_UNAVAILABLE_MESSAGE: &str = "Import from other apps is unavailable while Myra is connected to the local app-server daemon. Stop the daemon, restart Myra, and run /import.";
