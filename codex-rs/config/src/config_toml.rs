...
/// Base config deserialized from ~/.myra/config.toml.
...
pub struct ConfigToml {
    ...
    /// Defaults to `$CODEX_SQLITE_HOME` when set. Otherwise uses `$MYRA_HOME`.
    ...
    /// directories for `.myra` folders. Defaults to [".git"] when unset.
    #[serde(default)]
