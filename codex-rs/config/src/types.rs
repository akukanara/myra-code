...
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in MYRA_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in MYRA_HOME.
    Auto,
...
pub struct Tui {
    ...
    /// Use `/theme` in the TUI or see `$MYRA_HOME/themes` for custom themes.
    #[serde(default)]
    pub theme: Option<String>,

    /// Pet id to preselect in the terminal pet picker.
    ///
    /// Custom pet ids resolve against MYRA_HOME/pets/<pet-id>/pet.json.
    #[serde(default)]
