...
def load_json_object(path: Path, errors: list[str]) -> dict[str, Any] | None:
    if not path.is_file():
        errors.append("missing `.myra-plugin/plugin.json`")
        return None
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except OSError:
        errors.append("unable to read `.myra-plugin/plugin.json`")
        return None
    except json.JSONDecodeError:
        errors.append("`.myra-plugin/plugin.json` must be valid JSON")
        return None
    if not isinstance(payload, dict):
        errors.append("`.myra-plugin/plugin.json` must contain a JSON object")
        return None
    return payload
