...
Set a stable path to the skill CLI (default `MYRA_HOME` is `~/.myra`):

```
export MYRA_HOME="${MYRA_HOME:-$HOME/.myra}"
export IMAGE_GEN="$MYRA_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Built-in-first transparent image workflow: `SKILL.md` and `$MYRA_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py`
