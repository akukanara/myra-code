...
- "Where should I create this skill? If you do not have a preference, I will place it in `$MYRA_HOME/skills` (or `~/.myra/skills` when `MYRA_HOME` is unset) so Codex can discover it automatically."
...
Before running `init_skill.py`, ask where the user wants the skill created. If they do not specify a location, default to `$MYRA_HOME/skills`; when `MYRA_HOME` is unset, fall back to `~/.myra/skills` so the skill is auto-discovered.
...
```bash
scripts/init_skill.py my-skill --path "${MYRA_HOME:-$HOME/.myra}/skills"
scripts/init_skill.py my-skill --path "${MYRA_HOME:-$HOME/.myra}/skills" --resources scripts,references
