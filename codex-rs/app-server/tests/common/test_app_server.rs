...
pub struct TestAppServer {
    ...
    // removing an owned MYRA_HOME that may still be its cwd on Windows.
...
impl TestAppServer {
    /// Starts building a server with a temporary MYRA_HOME and the standard
...
impl TestAppServerBuilder {
    /// Uses this existing MYRA_HOME instead of a temporary one.
    ...
    /// Builds a server with a temporary MYRA_HOME and automatic environment
    /// by default.
