...
docker exec --user root "$CONTAINER_NAME" bash -c "mkdir -p /etc/myra"
for domain in $OPENAI_ALLOWED_DOMAINS; do
  ...
  echo "$domain" | docker exec --user root -i "$CONTAINER_NAME" bash -c "cat >> /etc/myra/allowed_domains.txt"
done

# Set proper permissions on the domains file
docker exec --user root "$CONTAINER_NAME" bash -c "chmod 444 /etc/myra/allowed_domains.txt && chown root:root /etc/myra/allowed_domains.txt"
