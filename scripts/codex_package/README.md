...
The builder creates a canonical Myra package directory:
...
│   ├── myra-command-runner.exe          # Windows only
│   └── myra-windows-sandbox-setup.exe   # Windows only
└── myra-path
    └── rg[.exe]
```
...
host platform. On Linux, that default is a musl target to match Myra release
...
`myra` and `myra-app-server`. The `version` field in `myra-package.json` is
...
- all targets: `myra-code-mode-host`, unless `--code-mode-host-bin` is provided
- Linux targets: `bwrap`, unless `--bwrap-bin` is provided
- Windows targets: `myra-command-runner` and `myra-windows-sandbox-setup`,
...
`--myra-command-runner-bin` plus `--myra-windows-sandbox-setup-bin` for
...
downloads and verifies the matching Myra-built V8 release pair before invoking
...
under `$TMPDIR/myra-package/<target>-rg` and are reused only after the recorded
...
manifest at `scripts/codex_package/myra-zsh` when the selected target has a
matching prebuilt artifact. Downloaded archives are cached under
`$TMPDIR/myra-package/<target>-zsh` and installed at
`myra-resources/zsh/bin/zsh`. Pass `--zsh-bin` to package a prebuilt, signed
