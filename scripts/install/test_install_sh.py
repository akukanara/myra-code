...
class InstallShTest(unittest.TestCase):
    def test_metadata_fetch_failure_is_not_reported_as_missing_assets(self) -> None:
        ...
        self.assertEqual(
            requests,
            [
                "https://api.github.com/repos/akukanara/myra-code/releases/tags/"
                f"rust-v{VERSION}"
            ],
        )
        self.assertIn(
            f"Could not fetch GitHub release metadata for Myra {VERSION}",
            result.stderr,
        )
        self.assertNotIn("Could not find Myra package", result.stderr)
    ...
    def test_exact_release_opt_out_uses_github_metadata_once(self) -> None:
        ...
        self.assertEqual(
            ...
            [
                "https://api.github.com/repos/akukanara/myra-code/releases/tags/"
                f"rust-v{VERSION}",
                "https://github.com/akukanara/myra-code/releases/download/"
                f"rust-v{VERSION}/myra-package_SHA256SUMS",
            ],
        )
    ...
    def test_json_like_strings_and_nested_fields_do_not_define_assets(self) -> None:
        ...
        self.assertIn("/myra-npm-", requests[1])
        self.assertNotIn("myra-package_SHA256SUMS", requests[1])

    def test_macos_install_exposes_code_mode_host_beside_myra(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            current = root / "myra-home" / "packages" / "standalone" / "current"
            myra_path = install_bin / "myra"
            host_path = install_bin / "myra-code-mode-host"
            self.assertEqual(os.readlink(myra_path), str(current / "bin" / "myra"))
            self.assertEqual(
                os.readlink(host_path),
                str(current / "bin" / "myra-code-mode-host"),
            )
    ...
    def test_releases_latest_installs_verified_package_by_default(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                requests,
                [
                    "https://releases.myralith.com/myra/channels/latest",
                    f"https://releases.myralith.com/myra/releases/{VERSION}/myra-package_SHA256SUMS",
                    f"https://releases.myralith.com/myra/releases/{VERSION}/myra-package-aarch64-apple-darwin.tar.gz",
                ],
            )
            ...
            "invalid_checksum_digest": json.dumps(
                {
                    ...
                    "assets": [
                        {
                            "name": "myra-package-aarch64-apple-darwin.tar.gz",
                            "digest": "sha256:" + "a" * 64,
                        },
                        {
                            "name": "myra-package_SHA256SUMS",
                            "digest": "sha256:" + "z" * 64,
                        },
                    ],
                }
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    self.assertEqual(
                        ...
                        [
                            ...
                            f"rust-v{VERSION}/myra-package_SHA256SUMS",
                            "https://github.com/akukanara/myra-code/releases/download/"
                            f"rust-v{VERSION}/myra-package-aarch64-apple-darwin.tar.gz",
                        ],
                    )
    ...
    def test_releases_exact_metadata_version_mismatch_falls_back_to_github(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                requests,
                [
                    f"https://releases.myralith.com/myra/releases/{VERSION}/release.json",
                    "https://api.github.com/repos/akukanara/myra-code/releases/tags/"
                    f"rust-v{VERSION}",
                    "https://github.com/akukanara/myra-code/releases/download/"
                    f"rust-v{VERSION}/myra-package_SHA256SUMS",
    ...
    def test_releases_wrong_checksum_digest_uses_github_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for release_asset in mirror_metadata["assets"]:
                if release_asset["name"] == "myra-package_SHA256SUMS":
    ...
    def test_releases_incomplete_checksum_manifest_falls_back_to_github(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mirror_checksum_path.write_text(
                f"{'a' * 64}  myra-package-other-platform.tar.gz\n",
                encoding="utf-8",
            )
            mirror_metadata = json.loads(metadata_json)
            for release_asset in mirror_metadata["assets"]:
    ...
    def test_releases_exact_rejects_wrong_binary_version(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                requests,
                [
                    f"https://releases.myralith.com/myra/releases/{MISMATCH_VERSION}/release.json",
                    f"https://releases.myralith.com/myra/releases/{MISMATCH_VERSION}/myra-package_SHA256SUMS",
                    f"https://releases.myralith.com/myra/releases/{MISMATCH_VERSION}/myra-package-aarch64-apple-darwin.tar.gz",
                ],
            )
            self.assertIn(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                ...
                [
                    ...
                    f"rust-v{VERSION}",
                    "https://github.com/akukanara/myra-code/releases/download/"
                    f"rust-v{VERSION}/myra-npm-darwin-arm64-{VERSION}.tgz",
                ],
            )
            ...
            self.assertNotIn("Downloading Myra CLI", second_result.stdout)
    ...
    *,
    ...
    fake_curl.write_text(
        textwrap.dedent(
            ...
            printf '%s\n' "$url" >>"$MYRA_TEST_REQUEST_LOG"

            case "$url" in
              https://api.github.com/*)
                if [ "$MYRA_TEST_METADATA_FAILURE" = "1" ]; then
                  echo "curl: (22) The requested URL returned error: 403" >&2
                  exit 22
                fi
                printf '%s\n' "$MYRA_TEST_METADATA_JSON"
                ;;
              https://releases.myralith.com/myra/channels/latest|https://releases.myralith.com/myra/releases/*/release.json)
                if [ "$MYRA_TEST_RELEASES_MODE" = "channel_failure" ]; then
                  exit 22
                fi
                printf '%s\n' "$MYRA_TEST_RELEASES_METADATA_JSON"
                ;;
              https://releases.myralith.com/myra/releases/*/myra-package_SHA256SUMS)
                if [ "$MYRA_TEST_RELEASES_MODE" = "asset_fallback" ]; then
                  exit 22
                fi
                if [ "$MYRA_TEST_RELEASES_MODE" = "corrupt_assets" ] ||
                  [ "$MYRA_TEST_RELEASES_MODE" = "corrupt_checksum_and_github" ]; then
                  printf '<html>proxy error</html>\n' >"$output"
                  exit 0
                fi
                if [ -n "$MYRA_TEST_RELEASES_CHECKSUM_PATH" ]; then
                  cp "$MYRA_TEST_RELEASES_CHECKSUM_PATH" "$output"
                else
                  exit 22
                fi
                ;;
              https://releases.myralith.com/myra/releases/*/myra-package-*.tar.gz)
                if [ "$MYRA_TEST_RELEASES_MODE" = "asset_fallback" ]; then
                  exit 22
                fi
                if [ "$MYRA_TEST_RELEASES_MODE" = "corrupt_assets" ]; then
                  printf '<html>proxy error</html>\n' >"$output"
                  exit 0
                fi
                if [ -n "$MYRA_TEST_ARCHIVE_PATH" ]; then
                  cp "$MYRA_TEST_ARCHIVE_PATH" "$output"
                else
                  exit 22
                fi
                ;;
              https://github.com/akukanara/myra-code/releases/download/*/myra-package_SHA256SUMS)
                if [ "$MYRA_TEST_RELEASES_MODE" = "corrupt_checksum_and_github" ]; then
                  printf '<html>proxy error</html>\n' >"$output"
                  exit 0
                fi
                if [ -n "$MYRA_TEST_CHECKSUM_PATH" ]; then
                  cp "$MYRA_TEST_CHECKSUM_PATH" "$output"
                else
                  exit 22
                fi
                ;;
              https://github.com/akukanara/myra-code/releases/download/*/myra-package-*.tar.gz)
                if [ -n "$MYRA_TEST_ARCHIVE_PATH" ]; then
                  cp "$MYRA_TEST_ARCHIVE_PATH" "$output"
                else
                  exit 22
                ...
                ;;
              https://github.com/akukanara/myra-code/releases/download/*/myra-npm-*.tgz)
                if [ -n "$MYRA_TEST_LEGACY_ARCHIVE_PATH" ]; then
                  cp "$MYRA_TEST_LEGACY_ARCHIVE_PATH" "$output"
                else
                  exit 22
                fi
                ;;
    ...
    env.update(
        {
            "MYRA_HOME": str(root / "myra-home"),
            "MYRA_INSTALL_DIR": str(root / "install-bin"),
            "MYRA_NON_INTERACTIVE": "1",
            "MYRA_RELEASE": release,
            "MYRA_TEST_ARCHIVE_PATH": str(archive_path or ""),
            "MYRA_TEST_CHECKSUM_PATH": str(checksum_path or ""),
            "MYRA_TEST_RELEASES_CHECKSUM_PATH": str(
                releases_checksum_path or checksum_path or ""
            ),
            "MYRA_TEST_LEGACY_ARCHIVE_PATH": str(legacy_archive_path or ""),
            "MYRA_TEST_METADATA_FAILURE": "1" if metadata_failure else "0",
            "MYRA_TEST_METADATA_JSON": (
            ...
            "MYRA_TEST_RELEASES_MODE": releases_mode,
            "MYRA_TEST_REQUEST_LOG": str(request_log),
            "HOME": str(home),
    ...
    if use_mirror is None:
        env.pop("MYRA_INSTALLER_USE_RELEASES_MYRALITH_COM", None)
    else:
        env["MYRA_INSTALLER_USE_RELEASES_MYRALITH_COM"] = (
...
def create_package_release(
    ...
    *,
    ...
    (package_dir / "myra-package.json").write_text("{}\n", encoding="utf-8")
    write_executable(
        package_dir / "bin" / "myra",
        f"#!/bin/sh\nprintf 'myra-cli {VERSION}\\n'\n",
    )
    write_executable(
        package_dir / "bin" / "myra-code-mode-host",
        "#!/bin/sh\nexit 0\n",
    )
    write_executable(package_dir / "myra-path" / "rg", "#!/bin/sh\nexit 0\n")

    asset = "myra-package-aarch64-apple-darwin.tar.gz"
    ...
    checksum_path = root / "myra-package_SHA256SUMS"
...
def create_legacy_release(root: Path) -> tuple[Path, str]:
    ...
    (vendor_dir / "myra").mkdir(parents=True)
    (vendor_dir / "path").mkdir()
    write_executable(
        vendor_dir / "myra" / "myra",
        f"#!/bin/sh\nprintf 'myra-cli {VERSION}\\n'\n",
    )
    write_executable(vendor_dir / "path" / "rg", "#!/bin/sh\nexit 0\n")

    asset = f"myra-npm-darwin-arm64-{VERSION}.tgz"
    archive_path = root / asset
    with tarfile.open(archive_path, "w:gz") as archive:
...
def legacy_release_metadata_with_decoys() -> str:
    ...
    assets = [
        {
            "metadata": {
                "name": "myra-package-x86_64-unknown-linux-musl.tar.gz",
                "digest": fake_digest,
            },
            "digest": f"sha256:{'c' * 64}",
            "name": f"myra-npm-{target}-{VERSION}.tgz",
        }
    ...
    return json.dumps(
        {
            "body": (
                f'fake: {{"name":"myra-package_SHA256SUMS","digest":"{fake_digest}"}}'
            ),
            "assets": assets,
            "tag_name": f"rust-v{VERSION}",
        },
